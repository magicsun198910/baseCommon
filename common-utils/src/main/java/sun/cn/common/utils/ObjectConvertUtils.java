...
public class ObjectConvertUtils {
    ...
    public static <T> T mapperObject(Class<T> target,Object ... srcs) {
        Assert.notNull(srcs, "src is not null");
        T tg = null;
        try {
        ...
        try {
            ...
            if (fields != null && fields.length > 0) {
                for (Field field : fields) {
                    ...
                    try{
                    Method s = src.getClass().getDeclaredMethod(GET+StringUtils.capitalize(name));
                    Object value = s.invoke(src);
                    FieldMapper mapper = field.getAnnotation(FieldMapper.class);
                    boolean flag = true;
                    if (mapper != null) {
                        name = StringUtils.isNotEmpty(mapper.name())?mapper.name():name;
                        flag = mapper.isMapper();
                    }
                    if (flag && value != null) {
                        Class<?> cls = (mapper ==null)?Object.class:mapper.mapperCls();
                        Method t = target.getClass().getDeclaredMethod(SET+StringUtils.capitalize(name), (cls.getName().equals(Object.class.getName()))?field.getType():cls );
                        t.invoke(target, value);
                    }
                    }catch(NoSuchMethodException|InvocationTargetException e){
                    	logger.warn(String.format("noSuchMethod or setValue error method is set/get%s and type is %s", StringUtils.capitalize(name),field.getType()));
                    }
                }
            }
